...
export default function HeroLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function guard() {
      ...
      if (!me) {
        window.location.href = '/login?superadmin=1';
        return;
      }
      if (me.email.toLowerCase() !== 'zafer@zaferkaraca.net' || me.role !== 'superadmin') {
        window.location.href = '/';
        return;
      }
      setUser(me);
    ...
    return (
      <main className="min-h-screen px-4 py-8 sm:px-6">
        <section className="mx-auto max-w-xl surface-card rounded-2xl p-6">
          <p className="text-sm text-slate-500">Superadmin kontrolu...</p>
        </section>
      </main>
    );
  }
    ...
    <>
      <header className="sticky top-0 z-20 border-b border-slate-200/70 bg-white/80 px-4 py-2 backdrop-blur dark:border-slate-700/70 dark:bg-slate-950/70">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <p className="text-xs text-slate-500">{user?.email} · superadmin</p>
          <button
            onClick={() => {
              clearAuthToken();
              window.location.href = '/login?superadmin=1';
